...
class Basic(commands.Cog):
    ...
    @commands.command(
        name='colorme',
        description='Gives user role with given hex color',
    )
    async def colorme(self, ctx, hexvalue: str):
        try:
            hexvalue = hexvalue.lstrip('#')
            hexval = int(hexvalue,16)
            newRole = await ctx.guild.create_role(name='#'+hexvalue, color = discord.Color(hexval))
            await ctx.message.author.add_roles(newRole)
    
        except Exception:
            await ctx.send('Gimme a hex value color')
            return
